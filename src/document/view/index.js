  ...
  return (
    <div className="document-container">
      <div style={{ display: "flex", flexDirection: "row" }}>
            ...
            return (
              <div className="page-section" key={subSectionNode.uid}>
                ...
                {nodeImage && nodeImage.name ? (
                  <div style={{ width: "100%", borderRadius: 5 }}>
                    <img
                      src={`${process.env.REACT_APP_BASE_API_URL}/api/document/${document_id}/720/${nodeImage.name}`}
                      alt=""
                      width="100%"
                    />
                  </div>
         ...
         */}
        {!isMobile ? (
          <RightDocumentContainer node={activeNode} document_id={document_id} />
        ) : null}
      </div>
    </div>
  );
...
const RightDocumentContainer = ({ node, document_id }) => {
  return (
    <div style={{ width: "20%", paddingLeft: 15, paddingTop: 15 }}>
      <ul className="list-item" style={{ paddingLeft: 0, listStyle: "none" }}>
        <li>
          <LuFileEdit color="#2d2d2d" size={16} />
          <Link to={`/edit/document/${document_id}`}>Edit this page</Link>
        </li>
        <li>
