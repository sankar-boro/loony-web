...
import { ContentPolicy, PrivacyPolicy, UserAgreement } from '../assets/svgs/icons.tsx';
...
const Followed = ({ authContext }: { authContext: AuthContextProps }) => {
  const [canFollowTags, setCanFollowTags] = useState<{uid: number, name: string}[] | null>(null);
  ...
  const user_removed_a_followed_tag = (tag_id: number) => {
    axiosInstance.post(`/tag/user_removed_a_followed_tag`, { tag_id, user_id: authContext.user && authContext.user.uid });
  };
  return (
    <div>
...
const Recommended = ({ authContext }: { authContext: AuthContextProps }) => {
  const [followedTags, setFollowedTags] = useState<{uid: number, name: string}[] | null>(null);
...
export default function Navbar({ authContext }: { authContext: AuthContextProps }) {
  return (
