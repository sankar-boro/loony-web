...
import type { Auth, AfterImageSelect, CropImageMetadata, EditImageComponentProps } from "../types/index.ts";


export default function CreateNewDocument({ url, title, isMobile }: { url: string, title: string, isMobile: boolean }) {
      ...
      >
        ...
        <div style={{ padding: isMobile ? 0 : "0px 45px 15px 45px" }}>
          <div style={{ padding: "0px 10px" }}>
            ...
            {afterTmpImageUpload && !imageEdit ? (
              <img
                src={`${process.env.REACT_APP_BASE_API_URL}/api/tmp/${user?.uid}/340/${afterTmpImageUpload}`}
                alt="tmp file upload"
              />
            ) : null}
            <div className="form-section">
