...
import { AppContextProps } from 'context/AppContext.tsx';

const Route = ({ authContext, appContext }: { authContext: AuthContextProps, appContext: AppContextProps }): React.JSX.Element => {
  ...
  const props = { setMobileNavOpen, mobileNavOpen, isMobile, authContext, appContext };
  ...
  return (
    <>
      {appContext.alert && <Alert alert={appContext.alert} onClose={onCloseAlert} />}
      <Navigation auth={authContext} setMobileNavOpen={setMobileNavOpen} isMobile={isMobile} />
      {authContext.status === AUTHORIZED && (
        <Routes>
          ...
          <ReactRoute
            path='/create/book'
            element={<Create url={CREATE_BOOK} title='Create Book' {...props} />}
          />
          <ReactRoute
            path='/create/blog'
            element={<Create url={CREATE_BLOG} title='Create Blog' {...props} />}
          />
